...
class AutoPT_FRDS(AutoPT.AutoPT):
    """login/logout/getpage"""

    def __init__(self):
        super(AutoPT_FRDS, self).__init__('FRDS')
        self.autoptpage = AutoPT_Page_FRDS
    ...
    def judgetorrentok(self, page):
        if page.method == 0:
            ...
            if page.futherstamp != -1:
                return (page.futherstamp - time.time() > 5 * 60 * 60) and page.seeders < 40
            else:
                return page.seeders < 40
            ...
            for line in page.find('table', class_='torrents').find_all('tr'):
                if n == 0:
                    ...
                    if line.find('img', class_='pro_free2up') is not None and not self.config['onlyattendance']:
                        ...
                        n = 1
                    elif line.find('img', class_='pro_free') is not None and not self.config['onlyattendance']:
...
class AutoPT_Page_FRDS(AutoPT.AutoPT_Page):
    """Torrent Page Info"""
    ...
    def __init__(self, soup, method=0):
        ...
        :soup: Soup
        """
        super(AutoPT_Page_FRDS, self).__init__(soup, method)
        try:
            lefttimetext = soup.find('img', class_='pro_free2up')
            if lefttimetext is None:
                lefttimetext = soup.find('img', class_='pro_free')
            lefttimetext = lefttimetext.attrs['onmouseover']
            self.lefttime = BeautifulSoup(lefttimetext, 'lxml').find('span').text

...
class AutoPT_SOULVOICE(AutoPT.AutoPT):
    """login/logout/getpage"""

    def __init__(self):
        super(AutoPT_SOULVOICE, self).__init__('SOULVOICE')
        self.autoptpage = AutoPT_Page_SOULVOICE
    ...
    def judgetorrentok(self, page):
        if page.method == 0:
            if page.futherstamp != -1:
                return (page.futherstamp - time.time() > 12 * 60 * 60) and page.seeders <= 1
            else:
...
class AutoPT_Page_SOULVOICE(AutoPT.AutoPT_Page):
    """Torrent Page Info"""
